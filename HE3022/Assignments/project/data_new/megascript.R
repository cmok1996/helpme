...
autoplot(seasadj(stl))+ylab("Index")+ggtitle("STL Decomposition - Seasonally Adjusted FSI (Overall)")

#Lag Plot#
lag.plot(fsitrain,lag=12,do.lines=FALSE)
#Overall Plot#
autoplot(FSI1,facets=TRUE)+ylab("Index")+ggtitle("Food & Beverage Services Index")
...
adf.test(diff(fsitrain,12),alternative="stationary")
adf.test(diff(diff(fsitrain,12),1),alternative="stationary")
kpss.test(diff(diff(fsitrain,12),1))
grid.arrange(ggPacf(diff(diff(fsitrain,12),1)), ggAcf(diff(diff(fsitrain,12),1)), ncol=2)
...
#autoplot(forecast(fit10,h=36))+ylab("Index")
#autoplot(fsi)+forecast::autolayer(forecast(fit10,h=36),series="ARIMA", PI=FALSE)+ggtitle("FSI")+ylab("Index")+guides(colour=guide_legend(title="Forecast Method"))
...
#Load data of predictor variables
...
#try multiple regression on 31 models
...
summary(fit16)
cbind("Regression Errors" = residuals(fit16, type="regression"), 
      "ARIMA errors" = residuals(fit16, type="innovation")) %>% autoplot(facets=TRUE)
checkresiduals(fit16) #p-value = 0.02788
jarque.bera.test(residuals(fit16, type="innovation"))
...
#checkresiduals(stlarima_seasadj_full)
seasonal_stl <- seasonal(stl(df_full[,1], s.window=13))
stlarima_full <- stlarima_seasadj_full$fitted + seasonal_stl
#accuracy(stlarima_full, df_full[,1])
...
##Forecasting March using known predictor variables
...
xreg_forecast <- cbind("rsi" = stlf(rsi_chained, h=1, method="naive")[["mean"]], "tourism" = 23.988, "cpi" = tail(cpi,1))
dynreg_forecast <- forecast(dynreg_full, xreg = xreg_forecast, h=1)
dynreg_forecast
autoplot(df_full[,"fsi"], series = "Actual") + 
  forecast::autolayer(dynreg_forecast, series = "Forecast") +
  ggtitle(" 1-month ahead ex-post forecast of FSI index") +
  ylab("Index")
...
#Grim scenario
fcast1_rsi <- stlf(rsi_chained, h=12, method = "naive") #Mac2020 - Feb2021
fcast1_cpi <- stlf(cpi,h=11, method = "naive") #Apr2020 - Mar2021
...
#Combined scenario
fcast_combine <- (0.7 * grim_forecast$mean) + (0.3 * good_forecast)
autoplot(df_full[,1], series = "Actual") + 
  forecast::autolayer(fcast_combine, series = "Forecast") + 
  ggtitle("12-months ahead weighted-average forecast for both scenarios") +
  ylab("Index")
fcast_combine 
...
colnames(df_check_full) = c("RSI", "Tourism", "CPI","IPI", "ULC", "AICc", "RMSE", "pVal", "RMSE_forecast")
write.csv(df_check_full, "df_check_full.csv")
df_check_full %>% arrange(RMSE_forecast) 
